...
**Current position**: O4 (KFP pipeline) → O5 (drift detection) → O6 (polish).  
**Current metrics**: ml_mean_f1=0.317 ✅, ml_mean_fatal_recall=0.524 ✅ — gates PASSED. crash-severity@champion v2 registered (winner=ml).
...
- [x] T129 [P] Update `src/tune/run.py`: import `OptunaTuner` alongside `HyperparamTuner` (keep both); use `OptunaTuner`; write all 5 best params + `dl.input_dim` to `params.yaml`.
- [x] T130 [P] Update `dvc.yaml` tune stage `params:` list — add `tune.optuna.*` keys.
- [x] T131 `uv run dvc repro tune` — study runs `n_trials`; MLflow shows runs tagged `trial_type=optuna`; `params.yaml` updated.
- [x] T132 `uv run dvc repro` (full downstream) — inspect `docs/evaluation_report.json`. **Gates pass → T051. Gates fail → M5 fixes.**

---

### Gate: Register

- [x] T051 `uv run dvc repro register` — prerequisite: gates passed from O3.5 or any M5 fix; `@champion` alias set; `mlflow.pyfunc.load_model("models:/crash-severity@champion")` loads; `model.predict(Z_test[:5])` returns shape `(5,)` with values in `{0, 1, 2}`.
...
- [x] T071 Rewrite `pipelines/kubeflow/pipeline.py` with 11 `@dsl.component` functions (added `train_gmm_op` in spec 003 T013) — sequential stages call `dvc repro <stage>`; parallel stages call `python -m src.<stage>.run` directly (avoids DVC rwlock contention between concurrent pods); mount PVC at `/app`; compile to `pipeline.yaml` via `_patch_pipeline_yaml()` which strips `pvcNameParameter` (unknown to KFP 2.2.0 driver, generated by kfp-kubernetes 2.16.1).
- [x] T072 `uv run python pipelines/kubeflow/pipeline.py` → `pipeline.yaml` created.
- [x] T073 Upload to KFP UI; start run; confirm all 10 steps with correct dependency arrows.
  <!-- Evidence: pipeline uploaded via kfp.Client (ID cfdc1484-143c-40c4-bca5-f77d67d327b6); run b84b337a created; Argo workflow crash-severity-pipeline-7z98h showed 27 nodes with correct DAG — featurize-op→[augment‖train_vae] (2 children), encode-op→[train_ml‖train_dl‖train_gmm] (3 children), register-op (0 children). Frontend image (ml-pipeline-ui) unavailable — gcr.io deleted all versions; DAG confirmed via Argo workflow nodes instead. Infrastructure changes: KFP upgraded v1.8.0→v2.2.0 (kustomize), MinIO patched to minio/minio:RELEASE.2021-02-14T04-01-33Z, argoexec patched to quay.io/argoproj/argoexec:v3.4.16. -->
- [x] T074 Inspect pod logs — `train_vae` ELBO visible; `train_ml` run tagged `orchestrator=kubeflow`.
  <!-- Evidence (run crash-severity-pipeline-9w4bl, run ID 4ac2e9c8-dbda-4059-a328-fec2b90d1306):
       Fixes applied to unblock:
       (1) pvcNameParameter: added _patch_pipeline_yaml() to pipeline.py — strips pvcNameParameter generated by kfp-kubernetes 2.16.1, keeps `constant` field recognised by KFP 2.2.0 driver.
       (2) PVC namespace: created k8s/pvc-kubeflow.yaml with mlops-portfolio-pv-kf PV + mlops-portfolio-pvc PVC in kubeflow ns, bound and confirmed.
       (3) Image pull: tagged mlops-portfolio:local (non-latest tag → IfNotPresent policy); rebuilt image with optuna 4.8.0 and orchestrator tag logic.
       (4) DVC rwlock contention: parallel stages (augment, train_vae, train_ml, train_dl, train_gmm) switched from `dvc repro` to `python -m src.<stage>.run` to avoid concurrent lock acquisition on shared PVC.
