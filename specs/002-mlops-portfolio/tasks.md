...
- [ ] T094 [US3] ⚠️ **BLOCKED — awaiting Phase 4A (augment)** Run `dvc repro encode` — confirm `Z_train_augmented.npy`, `Z_val.npy`, `Z_test.npy` written; verify `Z_train_augmented.shape[1] == latent_dim`; verify row count matches `X_train_augmented`. Cannot run until `X_train_augmented.npy` exists (produced by augment stage T108–T113) and `vae_encoder.pth` is rebuilt with T121+T106/T107 fixes.
- [ ] T095 [P] [US3] ⚠️ **BLOCKED — awaiting T094** Confirm val/test isolation: load `Z_val.npy` and `Z_test.npy` — assert shapes match original split sizes from featurize (no extra rows); assert `Z_val.shape[0] == len(X_val)` and `Z_test.shape[0] == len(X_test)`
